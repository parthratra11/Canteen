      ...
      try {
        ...
        if (data.status === "success") {
          setOutlets((prevOutlets) => {
            const existingOutlet = prevOutlets.find((outlet) => outlet.id === outletId);
            if (existingOutlet) {
              return prevOutlets.map((outlet) =>
                outlet.id === outletId ? { ...outlet, menu: data.product_info } : outlet
              );
            }
            return [...prevOutlets, { id: outletId, name: `Outlet ${outletId}`, menu: data.product_info }];
          });
        }
        setLoading(false);
      } catch (err) {
